# Shelf feature — verification matrix

Session 5 of [the Shelf plan](../../.claude/plans/i-want-to-add-buzzing-anchor.md) calls out 16 scenarios that gate "ready to ship". For each, this document records:
...
2. Open Gallery → pick any photo → Share → "FxFiles Shelf".
...
**Reproduce** —
1. Google Keep → write a note → Share to FxFiles Shelf.
...
**Expected** — select 5 images in Gallery → Share to FxFiles Shelf (uses ACTION_SEND_MULTIPLE). One descriptor JSON is written per transaction; 5 `ShelfItem` rows appear in `/dump`. Single notification "5 items dumped" (batched).
...
**Reproduce** —
1. Gallery → multi-select 3-5 photos → Share → FxFiles Shelf.
...
**Reproduce** —
1. Open a PDF in Google Drive → Share → FxFiles Shelf.
...
**Expected** — share to FxFiles Shelf from Photos → headless extension fires → "Queued 1 item — will upload when FxFiles next runs" notification appears within ~1 s. Reopen FxFiles → AppLifecycleState.resumed triggers the drain → ~5 s later "Shelfed: \<filename\>" notification replaces the queued one; the photo appears in `/dump`.
...
2. Photos → pick → Share → FxFiles Shelf.
...
**Reproduce** —
1. Safari → open a public page → Share → FxFiles Shelf.
...
**Coverage** — auto for the multi-mime drain path (`dump_ios_bridge_test.dart` "single descriptor with one file → ingest creates 1 ShelfItem"). Live mime detection is **device**.

**Reproduce** —
1. Files.app → pick any document (`.docx`, `.zip`, etc.) → Share → FxFiles Shelf.
...
**Expected** — sign out of FxFiles entirely. Share an image to FxFiles Shelf → it's ingested with status `pendingAuth` and a "Shelf saved — sign in to upload" notification appears. Sign back in → `AuthService._initializeFulaClient` fires `ShelfService.retryPending()` → the item transitions to `queued` and uploads immediately.
...
- The actual auth wiring (`unawaited(ShelfService.instance.retryPending())` inside `_initializeFulaClient`) is verified by `flutter build apk --debug` succeeding (compile-time check that the import + call are valid).
...
**Coverage** — auto for the ingest path (no network involved). Auto for SyncService retry behavior (pre-existing, not Shelf-specific). Live cycle is **device**.
...
**Expected** — share the SAME image twice in a row → the second share is dedup'd (R8: contentSha candidate + size + full SHA-256 verification for files ≤50 MB; large media accepts candidate). No second `ShelfItem` row is created. UX surface: the duplicate notification still appears briefly (the receiver Activity can't know it's a dup before staging), but `/dump` shows only one tile.
...
**Expected** — tap any Shelf notification → the app opens at `/dump` (deep link `fxfiles://dump`). If the notification carries a specific item id, opens at `/dump/<id>` and dispatches to the appropriate viewer.

**Coverage** — auto:
- `ShelfDeepLinkController` emits the right path (verified via `deep_link_service` change in Session 3).
- `app.dart`'s `_navigateToShelf` subscribes to the stream and pushes the route.

End-to-end is **device**.

**Reproduce** —
1. Trigger a Shelfed notification (any of the above scenarios).
...
2. Open Gallery (push FxFiles to background) → share an image → tap "FxFiles Shelf".
...
**Reproduce** —
1. `adb logcat | grep -E "WorkManager|ShelfProcess"`.
...
| S6-6 | Cursor | `ShelfService.init()` (which binds the `SyncService` status listener) is only called lazily on first ingest. If `SyncService` finishes a queued-from-prior-session upload before that first ingest, `SyncStatus.synced` is lost and the `ShelfItem` stays `uploading` forever. | Added `await ShelfService.instance.init()` to `main.dart` right after `LocalStorageService.init()` — listener bound on every cold start, idempotent on subsequent calls. |
...
| Gemini | "Notification redundancy on iOS — showReceived after extension's queued" | **False positive** — `ShelfNotificationService.showReceived` is a no-op on iOS (early-returns under `!_isAndroidEnabled`). Verified in code. |
...
| Cursor | Silent Hive init failure → permanent empty Shelf UI with no error channel | **Accepted for v1** — failure mode is rare on modern Android/iOS; documented as a known limitation. |
...
| Codex | App Store `UIBackgroundModes: audio` requires justification | **Pre-existing** — declared for the existing `AudioService` (ryanheise audio playback), not Shelf. Mentioned in PR description per Session 4 plan. |

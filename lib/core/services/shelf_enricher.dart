...
/// Result emitted by [ShelfEnricher.enrich]. Any field may be null; the
/// UI falls back to `originalName` / a category icon / `size · category`
/// as appropriate.
class ShelfEnrichmentResult {
  final String? title;
  final String? description;
  final String? thumbnailPath;
  final List<String> mlLabels;
  final ShelfEnrichmentStatus status;

  const ShelfEnrichmentResult({
    this.title,
    this.description,
    this.thumbnailPath,
  ...
  static const ShelfEnrichmentResult failed =
      ShelfEnrichmentResult(status: ShelfEnrichmentStatus.failed);
}

/// Generates auto-title / auto-description / thumbnail for a
/// [ShelfItem] using on-device tools only (per the plan's "no large
/// LLM" constraint).
///
...
/// for ML Kit / `video_thumbnail` / canvas. The ShelfService schedules
...
class ShelfEnricher {
  ShelfEnricher._();
  static final ShelfEnricher instance = ShelfEnricher._();
  ...
  static const String _kLinkUserAgent = 'FxFiles-Shelf/1.0';
  ...
  /// final [ShelfEnrichmentStatus].
  Future<ShelfEnrichmentResult> enrich(ShelfItem item) async {
    try {
      ...
      debugPrint('ShelfEnricher.enrich(${item.id}) failed: $e\n$st');
      return ShelfEnrichmentResult.failed;
    }
  }
  ...
  Future<ShelfEnrichmentResult> _enrichLink(ShelfItem item) async {
    final raw = item.textPayload?.trim();
    if (raw == null || raw.isEmpty) {
      return ShelfEnrichmentResult(
    ...
    //    Twitterbot UA — anonymous FxFiles-Shelf UA gets blank HTML.
    ...
    //  * Everything else: default FxFiles-Shelf/1.0.
  ...
  Future<ShelfEnrichmentResult> _enrichNote(ShelfItem item) async {
  ...
  Future<ShelfEnrichmentResult> _enrichImage(ShelfItem item) async {
    ...
    try {
      ...
      debugPrint('ShelfEnricher: ML Kit labeling failed: $e');
    ...
    try {
      ...
      debugPrint('ShelfEnricher: image thumb failed: $e');
    }

    String title;
    if (item.category == ShelfCategory.screenshot) {
      title = 'Screenshot';
  ...
  Future<ShelfEnrichmentResult> _enrichVideo(ShelfItem item) async {
    final file = File(item.localCachePath);
    if (!await file.exists()) return ShelfEnrichmentResult.failed;
    ...
    try {
      ...
      debugPrint('ShelfEnricher: video thumb failed: $e');
    }
  ...
  ShelfEnrichmentResult _enrichAudio(ShelfItem item) {
    return ShelfEnrichmentResult(
      title: p.basenameWithoutExtension(item.originalName),
      description:
          '${_formatBytes(item.sizeBytes)} · ${_categoryLabel(item.category)}',
  ...
  ShelfEnrichmentResult _enrichDocument(ShelfItem item) {
    return ShelfEnrichmentResult(
      title: p.basenameWithoutExtension(item.originalName),
      description:
  ...
  ShelfEnrichmentResult _enrichFile(ShelfItem item) {
  ...
  Future<bool> _isPublicHttpsTarget(Uri uri) async {
    ...
    try {
      ...
      debugPrint('ShelfEnricher: DNS lookup failed for ${uri.host}: $e');
      return false;
    }
  ...
  /// Optional [userAgent] overrides the default `FxFiles-Shelf/1.0` —
  ...
  Future<_LinkMetadata> _fetchLinkMetadata(
    ...
    try {
      ...
      debugPrint('ShelfEnricher: link metadata fetch failed for $uri: $e');
      return const _LinkMetadata();
    } finally {
  ...
  Future<String?> _fetchAndCacheLinkImage(
    ...
    try {
      ...
      debugPrint('ShelfEnricher: image fetch failed for $imageUrl: $e');
      return null;
    } finally {
      if (ownsClient) client.close();
    }
  ...
  Future<_LinkMetadata> _fetchTweetSyndication(String tweetId) async {
    ...
    try {
      ...
      debugPrint('ShelfEnricher: tweet syndication failed: $e');
      return const _LinkMetadata();
    } finally {
      if (ownsClient) client.close();
    }
  ...
  Future<String?> _readTextFile(String path) async {
    try {
      ...
      debugPrint('ShelfEnricher: text read failed for $path: $e');
      return null;
    }
  }

  String _categoryLabel(ShelfCategory category) {
    switch (category) {
      case ShelfCategory.link:
        return 'Link';
      case ShelfCategory.note:

...
/// through the standard ingest pipeline (`DumpService.ingestAndSchedule`).
/// Tags are attached to the resulting [DumpItem] via a synthetic URI
/// `dump://<id>` (revision H6 — keeps the tag link stable across cache
/// GC / file moves).
class DumpAddNoteScreen extends ConsumerStatefulWidget {
  const DumpAddNoteScreen({super.key});

  @override
  ConsumerState<DumpAddNoteScreen> createState() => _DumpAddNoteScreenState();
}

class _DumpAddNoteScreenState extends ConsumerState<DumpAddNoteScreen> {

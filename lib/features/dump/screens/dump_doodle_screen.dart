...
/// pipeline.
///
/// Hardening (per Dump plan Session 3b):
...
class DumpDoodleScreen extends ConsumerStatefulWidget {
  /// File path of the captured photo (e.g. from `image_picker`).
  final String capturedPath;

  const DumpDoodleScreen({super.key, required this.capturedPath});

  @override
  ConsumerState<DumpDoodleScreen> createState() => _DumpDoodleScreenState();
}

class _DumpDoodleScreenState extends ConsumerState<DumpDoodleScreen> {
  ...
  Future<void> _prepareWorkingImage() async {
    try {
      ...
      try {
        ...
        debugPrint('DumpDoodleScreen: decode failed: $e');
      }
      if (decoded == null) {
      ...
      debugPrint('DumpDoodleScreen prep failed: $e\n$st');
      if (!mounted) return;
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      try {
        ...
        debugPrint('DumpDoodleScreen: toImage failed: $e');
      }

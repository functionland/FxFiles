...
/// ShelfScreen so an Android Activity-death during capture doesn't lose
/// the photo.
class ShelfAddSheet extends ConsumerWidget {
  const ShelfAddSheet({super.key});
      ...
      child: Column(
        ...
        children: [
          ListTile(
            ...
            subtitle: const Text('Type some text — saved to your Shelf'),
  ...
  Future<void> _takePhoto(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      context.push('/shelf/doodle', extra: xfile.path);
    } catch (e) {
      debugPrint('ShelfAddSheet camera failed: $e');
      if (context.mounted) {
  ...
  Future<void> _importFile(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      debugPrint('ShelfAddSheet pickFiles failed: $e');
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      debugPrint('ShelfAddSheet staging failed: $e');
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
...
/// Helper used by `ShelfScreen.initState` to drain any photo that was
/// lost when Android killed the activity during `pickImage` (H1).
class ShelfLostDataHandler {
  ShelfLostDataHandler._();
  static final ShelfLostDataHandler instance = ShelfLostDataHandler._();
  ...
  Future<String?> retrievePendingPhoto() async {
    ...
    try {
      ...
      debugPrint('ShelfLostDataHandler.retrievePendingPhoto failed: $e');
      return null;
    }
  }
}

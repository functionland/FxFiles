// Unit tests for `DumpItem` — `copyWith` semantics and the
...
DumpItem _sampleItem({
  ...
  DumpEnrichmentStatus enrichmentStatus = DumpEnrichmentStatus.pending,
}) {
  return DumpItem(
    id: id,
...
void main() {
  group('DumpItem.copyWith', () {
    test('returns a new instance with the changed field updated', () {
      final original = _sampleItem();
      final updated = original.copyWith(uploadStatus: DumpUploadStatus.uploaded);

      expect(updated.uploadStatus, DumpUploadStatus.uploaded);
    ...
    test('does not mutate the original instance', () {
      final original = _sampleItem(uploadStatus: DumpUploadStatus.queued);
      final updated = original.copyWith(uploadStatus: DumpUploadStatus.failed);

      expect(original.uploadStatus, DumpUploadStatus.queued);
      expect(updated.uploadStatus, DumpUploadStatus.failed);
    });
  });

  group('DumpItem defaults', () {
    test('uploadStatus defaults to queued', () {
      ...
      expect(item.uploadStatus, DumpUploadStatus.queued);
    });
    ...
    test('enrichmentStatus defaults to pending', () {
      ...
      expect(item.enrichmentStatus, DumpEnrichmentStatus.pending);
    });
  ...
  group('DumpItem boolean status getters', () {
    test('isUploaded / isQueued / isPendingAuth / hasFailed', () {
      expect(
        _sampleItem(uploadStatus: DumpUploadStatus.uploaded).isUploaded,
        isTrue,
      );
      expect(
        _sampleItem(uploadStatus: DumpUploadStatus.queued).isQueued,
        isTrue,
      );
      expect(
        _sampleItem(uploadStatus: DumpUploadStatus.pendingAuth).isPendingAuth,
        isTrue,
      );
      expect(
        _sampleItem(uploadStatus: DumpUploadStatus.failed).hasFailed,
        isTrue,
      );
      expect(
        _sampleItem(uploadStatus: DumpUploadStatus.uploading).isUploading,
        isTrue,
      );
    });
  });
  ...
  group('Hive TypeAdapter round-trip', () {
    ...
    test('every field survives put/get with non-default values', () async {
      final box = await Hive.openBox<DumpItem>('roundtrip_test');
      final original = _sampleItem(
        id: 'roundtrip-1',
    ...
    test('every DumpCategory enum value survives round-trip', () async {
      final box = await Hive.openBox<DumpItem>('roundtrip_cats');
      for (final cat in DumpCategory.values) {
    ...
    test('every DumpUploadStatus enum value survives round-trip', () async {
      final box = await Hive.openBox<DumpItem>('roundtrip_upload');
      for (final s in DumpUploadStatus.values) {
    ...
    test('every DumpEnrichmentStatus enum value survives round-trip', () async {
      final box = await Hive.openBox<DumpItem>('roundtrip_enrich');
      for (final s in DumpEnrichmentStatus.values) {

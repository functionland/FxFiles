// Tests for DumpIosBridge — verifies that descriptor maps coming from
// the AppDelegate's `drainAppGroupContainer` MethodChannel are
// correctly funneled into DumpService.ingestAndSchedule, including
// the per-txn paths / mimes / originalNames / optional textPayload
// mapping.
//
// Heavy collaborators (FulaApiService / SyncService / AuthService)
// are stubbed via the DumpService test seams introduced in Session
...
void main() {
  ...
  setUp(() async {
    ...
    calls = <MethodCall>[];
    mockResponse = null;
    DumpIosBridge.debugForceIos = true;
    DumpNotificationService.debugForceAndroid = true;
    TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
        .setMockMethodCallHandler(
      MethodChannel(DumpIosBridge.channelName),
      (call) async {
        calls.add(call);
        return mockResponse;
      },
    );
    // Mute the notification channel — DumpService.ingestAndSchedule
  ...
  tearDown(() async {
    DumpIosBridge.debugForceIos = null;
  ...
  test('returns 0 when channel returns null', () async {
    mockResponse = null;
    final count = await DumpIosBridge.instance.drainAppGroupContainer();
    expect(count, 0);
  ...
  test('single descriptor with one file → ingest creates 1 DumpItem',
      () async {
    ...
    expect(all.first.category, DumpCategory.image);
  ...
  test('descriptor with textPayload preserves it', () async {
    ...
    final count = await DumpIosBridge.instance.drainAppGroupContainer();
    expect(count, 1);
    final created = DumpStorageService.instance.getAll().single;
    expect(created.textPayload, 'https://example.com');
    expect(created.category, DumpCategory.link);
  });
  ...
  test('descriptor with missing paths is skipped, not errored', () async {
    ...
    final count = await DumpIosBridge.instance.drainAppGroupContainer();
    expect(count, 1);
    expect(DumpStorageService.instance.getAll().single.originalName,
        'present.bin');
  });

  test('non-iOS host returns 0 without calling the channel', () async {
    DumpIosBridge.debugForceIos = false;
    mockResponse = <Object?>[
      <String, Object?>{
        'txnId': 'txn-x',

// Widget tests for ShelfScreen — empty / populated / search / filtered
...
// the test-completion window), `shelfItemsProvider` is overridden
...
GoRouter _router() {
  return GoRouter(
    ...
    routes: [
      GoRoute(path: '/shelf', builder: (_, __) => const ShelfScreen()),
      GoRoute(
        path: '/shelf/:id',
        builder: (_, state) =>
...
Future<void> pump(
  WidgetTester tester, {
  List<ShelfItem> items = const <ShelfItem>[],
}) async {
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        shelfItemsProvider.overrideWith(
          (ref) => Stream<List<ShelfItem>>.value(items),
        ),
      ],
...
void main() {
  ...
  testWidgets('tapping a tile pushes /shelf/:id', (tester) async {
    await pump(tester, items: [

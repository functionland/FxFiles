// Widget tests for ShelfTile — fallback rendering + upload-status
// overlays.
...
void main() {
  ...
  testWidgets('renders a CategoryPlaceholder when no thumbnail',
      (tester) async {
    await _pump(tester, ShelfTile(item: _item()));
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await _pump(
        tester,
        ShelfTile(item: _item(thumbnailPath: thumb.path)),
      );
  ...
  testWidgets('uploaded status shows no badge', (tester) async {
    await _pump(
      tester,
      ShelfTile(
        item: _item(uploadStatus: ShelfUploadStatus.uploaded),
      ),
    );
  ...
  testWidgets('queued status shows progress spinner badge', (tester) async {
    await _pump(
      tester,
      ShelfTile(
        item: _item(uploadStatus: ShelfUploadStatus.queued),
      ),
    );
  ...
  testWidgets('failed status shows an error icon badge', (tester) async {
    await _pump(
      tester,
      ShelfTile(
        item: _item(uploadStatus: ShelfUploadStatus.failed),
      ),
    );
  ...
  testWidgets('tap fires onTap', (tester) async {
    ...
    await _pump(
      tester,
      ShelfTile(item: _item(), onTap: () => tapped++),
    );
    // The tile now contains nested InkWells (the 3-dot actions menu's
    // IconButton ships its own InkResponse); the outermost InkWell is
    // the tile body that carries `onTap`. `.first` walks top-down.
  ...
  group('tag chip row', () {
    ...
    testWidgets('no tags → no tag-chip row', (tester) async {
      await _pump(tester, ShelfTile(item: _item()), tagsForFile: const []);
